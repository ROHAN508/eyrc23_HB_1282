...
class HBControl(Node):
    ...
    def __init__(self):
        ...
        self.subscription_bot = self.create_subscription(Goal,'hb_bot_3/goal', self.goalCallBack1, 10) 
        self.startrun = self.create_subscription(Bool,"/startrun", self.startCallBack1, 10)

        

        self.sub_bot = self.create_subscription(Pose2D, "/pen3_pose", self.Callback1, 10)
        self.twist_1 =  Float64MultiArray()
        
        self.stop_pub_1 = self.create_publisher(Bool, "/stop_bot3", 10)
        self.stop_bot = Bool()
        ...
        self.pub_1 = self.create_publisher(Float64MultiArray, '/map3', 10)
        self.pen_pub1 = self.create_publisher(Bool, '/pen3_down', 10)
        self.pen1=Bool()
...
class HBControlimg(Node):
    ...
    def __init__(self):
        ...
        self.subscription_bot3 = self.create_subscription(Goal,'hb_bot_3/goal_img', self.goalCallBack1, 10) 
        self.run_complete=self.create_subscription(Bool,'/pen3_complete',self.completeCallback,10)

        self.sub_bot_1 = self.create_subscription(Pose2D, "/pen3_pose", self.Callback1, 10)
        self.twist_1 =  Float64MultiArray()

        self.stop_pub_2 = self.create_publisher(Bool, "/stop_bot3", 10)
        self.stop_bot = Bool()
        self.stop_bot.data = False
...
def main(args=None):
    global i,func_complete,image_complete
    ...
    ##if mode is function
    while True: 
        global func_complete,image_complete       
        if mode==0 and func_complete==False:
            # rclpy.init(args=args)
            ...
            # rclpy.shutdown()
        ##image mode
        if mode==1 and image_complete==False:
            # rclpy.init(args=args)
        ...
        if image_complete==True and func_complete==True:
            rclpy.shutdown()
            break
