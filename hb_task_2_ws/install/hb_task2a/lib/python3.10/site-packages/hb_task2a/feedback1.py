...
class ArUcoDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        if a == 0:
            # self.get_logger().info(f'{self.req_list}')
            self.req_list_1 = []
            self.dist_list=[]
            for i in range(0, len(self.req_list)):
                for j in range(i+1, len(self.req_list)):
                    self.dist= ((self.req_list[i][1] - self.req_list[j][1])**2 + (self.req_list[i][2] - self.req_list[j][2])**2)**0.5
                    self.req_list_1.append([self.req_list[i][0][0], self.req_list[j][0][0], self.dist])
                    self.dist_list.append(self.dist)

            # self.get_logger().info(f'ids and dist: {self.req_list_1}')
            # self.get_logger().info(f'dist: {self.dist_list}')
            self.max_dist= max(self.dist_list)
            # self.get_logger().info(f'max dist: {self.max_dist}')
            self.error = self.max_dist*0.01
            # self.get_logger().info(f'ellol : {self.error}')
            self.square_side = self.max_dist/((2)**0.5)
            # self.get_logger().info(f'side : {self.square_side}')
            self.square_id = set()
            ...
            # self.get_logger().info(f'set : {self.square_id}')
            self.x1=0
            self.y1=0
            self.bot_info=[]
            ...
            # self.get_logger().info(f'center x : {self.x1}')
            # self.get_logger().info(f'center y : {self.y1}')
            ...
            # self.get_logger().info(f'center x : {self.x1}')
            # self.get_logger().info(f'center y : {self.y1}')
            # self.get_logger().info(f'bot info : {self.req_list[0][0]}')
            # self.get_logger().info(f'bot info : {self.bot_info}')
        for i in range(0, len(self.req_list)):
        ...
        self.pose_msg.x= (self.bot_info[1] - self.x1)/1.5
        self.pose_msg.y= ((self.bot_info[2] - self.y1)*(1))/1.5
        ...
        # self.get_logger().info(f'zip : {zip(ids, corners)}')
...
def cor(desired_id):
      # Replace with the actual ArUco ID you want to find
    cor_list=[]
