...
# Nodes:		Publshing nodes:HBController.pub_bot3_1
#                               HBController.pub_bot3_2
#                               HBController.pub_bot3_3
#              Subscribing nodes:HBController.sub_bot_3                       
...
class HBController3(Node):
    def __init__(self):
        ...
        self.pub_bot3_1 = self.create_publisher(Wrench, '/hb_bot_3/right_wheel_force', 10)
        self.pub_bot3_2 = self.create_publisher(Wrench, '/hb_bot_3/left_wheel_force', 10)
        self.pub_bot3_3 = self.create_publisher(Wrench, '/hb_bot_3/rear_wheel_force', 10)
    ...
    def goalCallBack1(self, msg1):
        self.bot_3_x_goal = msg1.x
        self.bot_3_y_goal = msg1.y
        self.bot_3_theta_goal = msg1.theta
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if hb_controller.bot_3_x_goal == []  and hb_controller.bot_3_x_goal == []:
            pass
        else:
                #########           GOAL POSE             #########
                x_goal= hb_controller.bot_3_x_goal[i]
                y_goal= hb_controller.bot_3_y_goal[i]
                theta_goal= hb_controller.bot_3_theta_goal
                ####################################################
                ...
                hb_controller.pub_bot3_1.publish(msg_1)

                msg_2 = Wrench()
                msg_2.force.x = 0.0
                msg_2.force.z = 0.0
                msg_2.force.y = req_forces[1]
                hb_controller.pub_bot3_2.publish(msg_2)

                msg_3 = Wrench()
                msg_3.force.x = 0.0
                msg_3.force.z = 0.0
                msg_3.force.y = req_forces[2]
                hb_controller.pub_bot3_3.publish(msg_3)
                # hb_controller.get_logger().info(f'{theta_goal}')
                ...
                if distance < distance_threshold  and q_b < theta_threshold:
                    i = i+1
                if i==len(hb_controller.bot_3_x_goal):
                    msg_1.force.y= 0.0
                    hb_controller.pub_bot3_1.publish(msg_1)
                    msg_2.force.y = 0.0
                    hb_controller.pub_bot3_2.publish(msg_2)
