...
# Nodes:		Publshing nodes:HBController.pub_bot1_1
#                               HBController.pub_bot1_2
#                               HBController.pub_bot1_3
#              Subscribing nodes:HBController.sub_bot_1                       
...
class HBController(Node):
    def __init__(self):
        ...
        self.subscription_bot1 = self.create_subscription(Goal,'hb_bot_1/goal', self.goalCallBack1, 10) 
        

        self.sub_bot_1 = self.create_subscription(Pose2D, '/detected_aruco_1', self.Callback1, 10)
        self.pub_bot1_1 = self.create_publisher(Wrench, '/hb_bot_1/right_wheel_force', 10)
        self.pub_bot1_2 = self.create_publisher(Wrench, '/hb_bot_1/left_wheel_force', 10)
        self.pub_bot1_3 = self.create_publisher(Wrench, '/hb_bot_1/rear_wheel_force', 10)
    ...
    def inverse_kinematics(self,xvel, yvel, ang_vel):
        ...
        ############################################
        # w1_vel = (1/r) * ((ang_vel*d) + (-0.5*xvel) + (0.866*yvel))
        # w2_vel = (1/r) * ((ang_vel*d) + (-0.5*xvel) + (-0.866*yvel))
        # w3_vel = (1/r) * ((ang_vel*d) + (1*xvel) + (0*yvel))
        w1_vel= (-0.33*xvel)+(0.58*yvel)+(0.33*ang_vel)
        w2_vel= (-0.33*xvel)+(-0.58*yvel)+(0.33*ang_vel)
        w3_vel= (0.66666*xvel)+(0.33333*ang_vel)
        
#         w1_vel = (1/r) * ( (w*d) + (-0.5*xvel) + (0.866*yvel) );
#         w2_vel = (1/r) * ( (w*d) + (-0.5*xvel) + (-0.866*yvel) );
#         w3_vel = (1/r) * ( (w*d) + (1*xvel) + (0*yvel) );
#   Serial.println(w1_vel);
        return [w1_vel, w2_vel, w3_vel]
