    ...
    while rclpy.ok():
        ...
        if ebot_controller.future.done():
            try:
                ...
                x_error = abs(x_goal - ebot_controller.hb_x)
                y_error = abs(y_goal - ebot_controller.hb_y)
                theta_error= abs(theta_goal - ebot_controller.hb_theta)
                ...
                if theta_error > 0.1:
                    ...
                    ebot_controller.vel.angular.z = 2*theta_error   
                    ebot_controller.pub.publish(ebot_controller.vel)
                if theta_error<= 0.1:
                    ebot_controller.vel.angular.z = 0  
                    ebot_controller.pub.publish(ebot_controller.vel)

                    
                if 0 <= theta_error < 0.1 and distance >= 0.1:
                    ebot_controller.vel.linear.x = 2*x_error
                    ebot_controller.vel.linear.y = 2*y_error
                    ebot_controller.pub.publish(ebot_controller.vel)

                if distance < 0.1:
                    ebot_controller.vel.linear.x = 0
                    ebot_controller.vel.linear.y = 0
                    ebot_controller.pub.publish(ebot_controller.vel)
            ...
            if distance < 0.1 and theta_error < 0.1:    
                ############     DO NOT MODIFY THIS       #########
