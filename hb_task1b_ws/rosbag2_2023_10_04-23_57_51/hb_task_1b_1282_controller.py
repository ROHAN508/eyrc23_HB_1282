    ...
    while rclpy.ok():
        ...
        if ebot_controller.future.done():
            try:
                ...
                x_error = (x_goal - ebot_controller.hb_x)
                y_error = (y_goal - ebot_controller.hb_y)
                theta_error= (theta_goal - ebot_controller.hb_theta)

                distance= ((x_error)**2 + (y_error)**2)**0.5
                if ebot_controller.hb_theta>=0:
                    deviation=ebot_controller.hb_theta
                if ebot_controller.hb_theta<0:
                    deviation=-(ebot_controller.hb_theta)    

                if abs(deviation) > threshold_theta:
                    ebot_controller.vel.linear.x = 0.0
                    ebot_controller.vel.linear.y = 0.0
                    ebot_controller.vel.angular.z = ebot_controller.t*deviation
                    ebot_controller.pub.publish(ebot_controller.vel)
                if abs(deviation)<= threshold_theta:
                    ebot_controller.vel.angular.z = 0.0  
                    ebot_controller.pub.publish(ebot_controller.vel)

                    
                if 0 <= abs(deviation) < threshold_theta and distance >= threshold:
                    ebot_controller.vel.linear.x = ebot_controller.p*x_error
                    ebot_controller.vel.linear.y = ebot_controller.p*y_error
                    ebot_controller.pub.publish(ebot_controller.vel)
                ...
                if distance < threshold:
                    ...
                    if abs(theta_error)>0.1:
                        ebot_controller.vel.angular.z = ebot_controller.t*theta_error
                    else:
                        ebot_controller.vel.angular.z=0.0
                    ebot_controller.pub.publish(ebot_controller.vel)    
            ...
            if distance < 0.12 and theta_error < 0.15:    
                ############     DO NOT MODIFY THIS       #########
